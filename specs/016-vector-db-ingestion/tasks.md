...
- [X] T002 Initialize Python project with UV using `uv init`
- [X] T003 Create pyproject.toml with project metadata and dependencies
- [X] T004 Add dependencies to pyproject.toml: beautifulsoup4, requests, cohere, qdrant-client, python-dotenv, tenacity
- [X] T005 Generate uv.lock file with `uv sync --dev`
- [X] T006 Create .env.example file with environment variable placeholders
- [X] T007 Create main.py with initial file structure and imports
...
- [X] T010 Create configuration loading function to read from .env file
- [X] T011 Implement utility functions for logging and progress tracking
- [X] T012 Create helper functions for token counting and text processing
- [X] T013 Set up Cohere client with API key from environment variables
- [X] T014 Set up Qdrant client with URL and API key from environment variables
- [X] T015 Implement retry mechanism with tenacity for API calls
- [X] T016 Create constants file for configuration defaults (chunk size, overlap, etc.)
...
- [X] T020 [US1] Implement function to fetch sitemap.xml from https://salmanpatnee.github.io/Physical-AI-Humanoid-Robotics/sitemap.xml
- [X] T021 [US1] Parse sitemap to extract all book page URLs
- [X] T022 [US1] Create function to fetch HTML content from a single URL using requests
- [X] T023 [P] [US1] Create function to extract readable content from HTML using BeautifulSoup and Docusaurus-specific selectors
- [X] T024 [P] [US1] Create function to chunk extracted content into 512-token segments with 50-token overlap
- [X] T025 [P] [US1] Create function to generate embeddings using Cohere for content chunks
- [X] T026 [US1] Create Qdrant collection for storing content chunks and embeddings
- [X] T027 [US1] Store chunk content, embeddings, and metadata in Qdrant with proper payload structure
- [X] T028 [US1] Implement progress tracking for ingestion process
- [X] T029 [US1] Add error handling for failed URL fetches, extraction, or embedding generation
- [X] T030 [US1] Implement graceful degradation for API limits and connection issues
- [X] T031 [US1] Create main ingestion function that orchestrates the entire pipeline
- [X] T032 [US1] Add command-line arguments for configuration (book URL, collection name, chunk size, overlap)
- [X] T033 [US1] Implement ingestion statistics reporting
...
- [X] T040 [US2] Implement semantic search function that takes query text and returns relevant chunks
- [X] T041 [US2] Create function to generate embedding for query text using Cohere
- [X] T042 [US2] Implement vector search in Qdrant using the query embedding
- [X] T043 [US2] Format search results with preserved metadata (URL, section title, etc.)
- [X] T044 [US2] Add search result ranking by similarity score
- [X] T045 [US2] Implement search result limiting (top N results)
- [X] T046 [US2] Add search API endpoint: POST /search
...
- [X] T050 [US3] Create function to validate content chunk metadata before storage
- [X] T051 [US3] Implement metadata extraction and validation from source pages
- [X] T052 [US3] Add metadata validation when retrieving content from Qdrant
- [X] T053 [US3] Create function to update/modify chunk metadata if needed
- [X] T054 [US3] Implement function to retrieve content by metadata criteria (URL, section, etc.)
- [X] T055 [US3] Add metadata consistency checks during ingestion
- [X] T056 [US3] Add API endpoint for metadata management: GET /ingestion-status/{job_id}
...
- [X] T060 Add comprehensive error logging and reporting
- [X] T061 Implement ingestion job status tracking with unique job IDs
- [X] T062 Add ingestion rate limiting to prevent overwhelming APIs
- [X] T063 Create ingestion progress API endpoint: GET /ingestion-status/{job_id}
- [X] T064 Add ingestion statistics and metrics
- [X] T065 Update README.md with complete usage instructions
- [X] T066 Add example scripts for common use cases
- [X] T067 Implement comprehensive input validation
- [X] T068 Add graceful handling for Qdrant Cloud Free Tier limits
- [X] T069 Update .env.example with all required variables and explanations
- [X] T070 Create documentation for troubleshooting common issues